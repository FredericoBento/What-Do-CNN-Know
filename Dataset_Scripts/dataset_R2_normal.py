...
folder = 'Datasets/Dataset_R2_Normal'
...
for j in range(2):
    ...
    mean = max_proportion / 2
    std_dev = (max_proportion - 1) / 6
    distribution = np.random.normal(mean, std_dev, int(size))
    distribution = np.clip(distribution, 1, max_proportion)
    for i in range(size):
